...
public class ItemRepository {
    ...
    public ItemRepository(JdbcTemplate jdbcTemplate, PessoaRepository pessoaRepository, PagamentoRepository pagamentoRepository, RelatorioRepository relatorioRepository) {
    ...
    public Page<Item> buscar(
        ...
        String pageSql =
                baseSelect + " WHERE i.id IN (" + inPlaceholders + ") ORDER BY i.descricao ";
    ...
    public void consumirItem(Item.Consumo.Request request) {
        ...
        if (temQuarto) {
            ...
            try {
                qtdQuarto = jdbcTemplate.queryForObject("""
                        SELECT quantidade_atual FROM quarto_item
                        WHERE fk_quarto = ? AND fk_item = ?
                        """, Integer.class, request.quarto().id(), request.item().id());
            ...
            if (qtdQuarto != null && qtdQuarto >= request.quantidade()) {
                jdbcTemplate.update("""
                        ...
                        WHERE fk_quarto = ? AND fk_item = ?
                        """, request.quantidade(), request.quarto().id(), request.item().id());
                return;
            }
        }
        ...
        jdbcTemplate.update("""
                ...
                WHERE fk_item = ?
                """, request.quantidade(), request.item().id());
    }
    ...
    public void cancelarConsumo(Long consumoId) {
        ...
        Long quartoId = row.get("fk_quarto") != null ? ((Number) row.get("fk_quarto")).longValue() : null;
    ...
    public List<Item.Consumo> listarConsumos(Pageable pageable) {
        ...
        return jdbcTemplate.query(sql, Item.Consumo.ROW_MAPPER,
    ...
    public Item.HistoricoEstoque.Estoque listarEstoque() {
        ...
        var categorias = rows.stream()
                .collect(java.util.stream.Collectors.groupingBy(r -> ((Number) r.get("categoria_id")).longValue(),
                        java.util.LinkedHashMap::new, java.util.stream.Collectors.toList()))
                .values().stream()
                .map(maps -> {
                    var first = maps.getFirst();

                    var itens = maps.stream()
                            .filter(row -> row.get("item_id") != null)
                            .map(row -> new Item.HistoricoEstoque.Estoque.CategoriaItem.ItemEstoque(
                                    ((Number) row.get("item_id")).longValue(),
                                    (String) row.get("item_descricao"),
                                    row.get("item_quantidade") != null ? ((Number) row.get("item_quantidade")).intValue() : 0,
                                    row.get("item_valor_venda") != null ? ((Number) row.get("item_valor_venda")).floatValue() : 0f))
                            .toList();

                    var itensComEstoque = maps.stream().filter(r -> r.get("item_id") != null).toList();
                    var dashboard = new Item.HistoricoEstoque.Estoque.CategoriaItem.Dashboard(
                            itensComEstoque.stream().mapToInt(r -> r.get("item_quantidade") != null ? ((Number) r.get("item_quantidade")).intValue() : 0).sum(),
                            (float) itensComEstoque.stream().mapToDouble(r -> ((Number) r.get("valor_investido")).doubleValue()).sum(),
                            (float) itensComEstoque.stream().mapToDouble(r -> ((Number) r.get("lucro_potencial")).doubleValue()).sum());
    ...
    public void registrarHistoricoReposicao(Item.HistoricoEstoque.Request request) {
        Long estoqueId;
        try {
            estoqueId = jdbcTemplate.queryForObject(
                    "SELECT id FROM estoque WHERE fk_item = ?", Long.class, request.item().id());
        } catch (EmptyResultDataAccessException e) {
            estoqueId = insertEstoque(new Item.ReporEstoque(
                    request.item(),
                    0,
