...
public class HospedagemRepository {
    ...
    public List<Hospedagem> buscar(
        ...
        if (statuses != null && !statuses.isEmpty()) {
            String placeholders = statuses.stream().map(s -> "?::hospedagem_status").collect(Collectors.joining(", "));
    ...
    public Long adicionarOrcamento(Orcamento.Request request, Long funcionarioId) {
        return jdbcTemplate.queryForObject(
                """
                        ...
                        returning id;
                        """,
                Long.class, request.nome_solicitante(), funcionarioId);
    }

    public void editarOrcamento(Orcamento.Request request, LocalDateTime checkin, LocalDateTime checkout) {
        jdbcTemplate.update(
                """
                        UPDATE orcamento
    ...
    public Orcamento buscarOrcamento(Long orcamentoId) {
        Orcamento orcamento =
                jdbcTemplate.queryForObject(SELECT_HOSPEDAGEM +
                                """
                                        LEFT JOIN hospedagem_orcamento ON hospedagem_orcamento.fk_hospedagem = hospedagem.id
                                        WHERE hospedagem_orcamento.fk_orcamento = ?
                                        """,
                        (rs, x) ->
                                new Orcamento(
                                        rs.getLong("id"),
        ...
        List<Hospedagem.PessoaHospedagemOrcamento> pessoas =
                jdbcTemplate.query(
                        """
                                ...
                                """,
                        (rs, x) ->
                                new Hospedagem.PessoaHospedagemOrcamento.Request(
                                        rs.getLong("id"),
