...
public class FuncionarioRepository {
    ...
    private static final ResultSetExtractor<List<Funcionario>> FUNCIONARIO_EXTRACTOR = rs -> {
        ...
        while (rs.next()) {
            ...
            if (tela != null) {
                ...
                if (permissao != null) {
                    ...
                    if (permissaoIdsPorFuncionarioTela.get(funcionarioId).get(tela.id()).add(permissao.id())) {
        ...
        for (Map.Entry<Long, Funcionario> entry : funcionarios.entrySet()) {
            ...
            Cargo cargo =
                    funcionarioBase.cargo() == null
                            ? null
                            : new Cargo(
                            funcionarioBase.cargo().id(),
                            funcionarioBase.cargo().descricao(),
                            telas);
    ...
    public Page<Funcionario> buscar(
        ...
        String pageSql =
                baseSelect
                        + " WHERE f.id IN (" + inPlaceholders + ") "
                        + " ORDER BY p.nome ASC, t.nome ASC, perm.permissao ASC ";

        List<Funcionario> content =
                jdbcTemplate.query(pageSql, FUNCIONARIO_EXTRACTOR, ids.toArray());
    ...
    public Funcionario insert(Funcionario.Request request) {
        var funcionario_id = jdbcTemplate.queryForObject(
            """
