...
public class HospedagemService {
        ...
        diarias.forEach(
                diaria -> {
                    log.info(
                            ...
                            hospedagemId, diaria.quarto_id());
                    if (isDiariaJaExiste(hospedagemId, diaria)) {
                        log.info(
                                "Diária [{}>{}] já existe para a hospedagem {} e quarto {}",
                                diaria.checkin(), diaria.checkout(), hospedagemId, diaria.quarto_id());
                        return;
                    }
                    if (!isQuartoDisponivel(
                        ...
                        log.info(
                                ...
                                diaria.checkin(), diaria.checkout(), diaria.quarto_id());
                    ...
                    diaria.pessoas().forEach(pessoaId -> {
                        ...
                        log.info(
                                ...
                                pessoaId, hospedagemId, diaria.quarto_id());
                    });
    ...
    private void vincularHospedagensOrcamento(Long orcamentoId, List<Long> hospedagensIds) {
        hospedagemRepository.vincularHospedagensOrcamento(hospedagensIds, orcamentoId);
    }

    // ── Orçamento ────────────────────────────────────────────────────────────────

    private Long adicionarOrcamento(Orcamento.Request orcamento) {
        validarCamposOrcamento(orcamento, false);
        var orcamentoId =  hospedagemRepository.adicionarOrcamento(orcamento, getFuncionarioId());
        orcamento.hospedagens().forEach(hospedagem -> {
            var newHospedagem = hospedagemRepository.insertHospedagem(hospedagem, getFuncionarioId());
            adicionarPessoasHospedagemOrcamento(newHospedagem.id(), hospedagem.pessoas_orcamento());
                ...
                request -> {
                    if (request == null)
                        throw new IllegalArgumentException(
                                "Requisição de pessoas no orçamento não informada");
                    if (isUpdate) {
                        if (request.id() == null || request.id() <= 0)
    ...
    public void editarOrcamento(Long orcamentoId, List<Hospedagem.Request> requests) {
        validarCamposOrcamento(request, true);
        removerTodasPessoasOrcamento(orcamentoRequest.id());
        adicionarPessoasHospedagemOrcamento(orcamentoRequest.id(), orcamentoRequest.pessoas());
        hospedagemRepository.editarOrcamento(
                orcamentoRequest,
                request.data_hora_checkin(),
                request.data_hora_checkout()
        );
        hospedagemRepository.editarHospedagem(request);
    }

    public void adicionarPessoasHospedagemOrcamento(Long hospedagemId, List<Hospedagem.PessoaHospedagemOrcamento.Request> pessoas) {
    ...
    public void removerTodasPessoasOrcamento(Long orcamentoId) {
        hospedagemRepository.removerTodasPessoasOrcamento(orcamentoId);
    }
    ...
    @Transactional
    public Hospedagem criarOrcamento(List<Hospedagem.Request> requests) {
        ...
        requests.forEach(request -> {
            ...
            if (request.data_hora_checkout() == null)
                throw new IllegalArgumentException(
                        "E necessário informar a data de checkout da hospedagem.");
            ...
            var insertRequest =
                    new Hospedagem.Request(
                            ...
                            request.observacao(),
                            request.valor_total(),
                            request.pessoas_orcamento(),
                            null,
                            null);
            Hospedagem hospedagem = hospedagemRepository.insertHospedagem(insertRequest, getFuncionarioId());
        })


        var orcamentosIds = adicionarOrcamentos(
                request
        );
        request
                .orcamentos()
                .forEach(
                        req ->
                                orcamentosIds.forEach(
                                        orcId -> adicionarPessoasHospedagemOrcamento(orcId, req.pessoas())));
        return hospedagem;
    }

    @Transactional
    public void cancelarOrcamento(Long hospedagemId, MotivoCancelamentoHospedagem.Request motivo) {
        ...
        alterarStatus(hospedagemId, Hospedagem.Status.ORCAMENTO_CANCELADO);
    }
    ...
    @Transactional
    public Hospedagem editarHospedagem(Long hospedagemId, Hospedagem.Request request) {
        hospedagemRepository.buscarPorId(hospedagemId);
        return withDetails(hospedagemRepository.editarHospedagem(hospedagemId, request));
    }
    ...
    private Hospedagem withDetails(Hospedagem h) {
        var diarias = listarDiarias(h.id());
        var consumos = buscarConsumosPorHospedagem(h.id());
        var pagamentos = pagamentoService.buscarPorHospedagemId(h.id());

        List<Orcamento> orcamentos;
        try {
            orcamentos = List.of(buscarOrcamento(h.id()));
        ...
        try {
            motivo = buscarMotivoCancelamento(h.id());
        } catch (EmptyResultDataAccessException e) {
            motivo = null;
        }
