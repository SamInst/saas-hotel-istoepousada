...
public class HospedagemService {
    ...
    public Boolean isQuartoDisponivel(
        ...
        var hospedagemConflito = hospedagemRepository.isQuartoDisponivel(
                quartoId,
                checkin,
                checkout,
    ...
    public void adicionarDiarias(Long hospedagemId, List<Hospedagem.Diaria.Request> diarias) {
        ...
        diarias.forEach(diaria -> {
            ...
            if (!isQuartoDisponivel(diaria.quarto_id(), diaria.checkin(), diaria.checkout(), hospedagemId)) {
                log.info("Diaria: [{}>{}] não disponivel para o quarto: {}. O apartamento se encontra indisponível ou ocupado por outra hospedagem.",
                        diaria.checkin(),
                        diaria.checkout(),
                        diaria.quarto_id()
                );
                throw new IllegalStateException("Diaria: [" + diaria.checkin() + ">"
                        + diaria.checkout() + "] não disponivel para o quarto: "
                        + diaria.quarto_id()
            ...
            calcularPrecoRequests.add(new CalcularPreco.Request(
                    diaria.quarto_id(),
        ...
        hospedagemRepository.adicionarDiarias(
                ...
                resultadoCalculo.getFirst().valor_total()
        );
    }
    ...
    public void removerPessoas(Long hospedagemId, List<Long> pessoasIds) {
        hospedagemRepository.buscarPorId(hospedagemId);
        List<Long> pessoasExistentes = hospedagemRepository.filtrarPessoasExistentes(hospedagemId, pessoasIds);
    ...
    public void adicionarPessoas(Long hospedagemId, List<Long> pessoasIds) {
        hospedagemRepository.buscarPorId(hospedagemId);
        List<Long> pessoasPendentes = hospedagemRepository.filtrarPessoasDuplicadas(hospedagemId, pessoasIds);
    ...
    public List<Long> adicionarOrcamentos(Long hospedagemId, List<Orcamento.Request> requests) {
        if (hospedagemId == null || hospedagemId <= 0 || hospedagemRepository.buscarPorId(hospedagemId) == null) {
    ...
    private void validarCamposOrcamento(List<Orcamento.Request> requests, Boolean isUpdate) {
        ...
        requests.forEach(request -> {
            ...
            if (request.categoria() == null) {
                ...
                throw new IllegalArgumentException("Categoria do solicitante não informado ou inválido");
            }
            if (request.checkin() == null) {
                ...
                throw new IllegalArgumentException("Data de checkin não pode ser anterior a data atual");
            }
            if (request.checkout() == null) {
                ...
                throw new IllegalArgumentException("Data de checkout não pode ser anterior a data atual");
            }
    ...
    private void validarCamposPessoasOrcamento(List<Orcamento.Pessoa.Request> pessoas, Boolean isUpdate) {
        pessoas.forEach(request -> {
            if (request == null) {
...
//                }
                ...
                case PERNOITE_FINALIZADA_PAGAMENTO_PENDENTE: {
                    alterarStatus(hospedagem.id(), Hospedagem.Status.PERNOITE_FINALIZADA_PAGAMENTO_PENDENTE);
                }
...
//            }
                default: {
                    throw new IllegalStateException("Status inválido para registro de request: " + request.status());
                }
            }
        });
    }
