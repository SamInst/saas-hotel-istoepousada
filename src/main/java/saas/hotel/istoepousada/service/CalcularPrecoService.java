...
public class CalcularPrecoService {
  ...
  public CalcularPrecoService(
      CategoriaRepository categoriaRepository, QuartoRepository quartoRepository) {
  ...
  public List<CalcularPreco.Resultado> calcularPreco(List<CalcularPreco.Request> requests) {
    ...
    List<CalcularPreco> resolved = requests.stream().map(this::calcularPorDataNascimento).toList();
    ...
    List<Long> categoriaIds =
        catInfoMap.values().stream().map(CategoriaCheckin::id).distinct().toList();
    ...
    Map<Long, Categoria.DayUseOperacao> sazonDayUse =
        (temDayUse && !sazonalidadeIds.isEmpty())
            ? categoriaRepository.buscaDayUseSazonalidade(sazonalidadeIds)
            : Map.of();
  ...
  private CalcularPreco calcularPorDataNascimento(CalcularPreco.Request request) {
    ...
    LocalDate dataRef =
        request.hora_inicio() != null
            ? request.hora_inicio().toLocalDate()
  ...
  private CalcularPreco.Resultado calcularPrecoUnico(
    ...
    if (request.hora_inicio() != null && request.hora_fim() != null) {
      return calcularDayUseUnico(
          request, categoriaCheckin, categoria, sazonalidades, sazonalidadeDayUse, quartoDesc);
    }
    ...
    LocalDate dataEntradaCalculo =
        mesmaData ? request.data_entrada().minusDays(1) : request.data_entrada();
    int noites =
    ...
    Categoria.Nome categoriaObj =
        new Categoria.Nome(categoriaCheckin.id(), categoriaCheckin.nome());
  ...
  private CalcularPreco.Resultado calcularDayUseUnico(
    ...
    return new CalcularPreco.Resultado(
        ...
        List.of(
            new CalcularPreco.ItemPreco(
                descricao, sazonNome, valorBase, acrescimo, null, valorTotal)));
  }
